...
describe('RelatedBenefits.vue', () => {
  ...
  it('각 카드에 상세 페이지 링크가 설정되어야 한다', () => {
    ...
    // 내부 상세 페이지 경로 확인
    expect(links[0].attributes('href')).toBe('/benefits/b1')
    expect(links[1].attributes('href')).toBe('/benefits/b2')
    expect(links[2].attributes('href')).toBe('/benefits/b3')
  })
