...
describe('SearchForm.vue', () => {
  ...
  it('카테고리와 대상조건 필터가 emit에 포함되어야 한다', async () => {
    ...
    // 폼 입력 (1995-06-15 → 만 30세)
    await wrapper.find('input[name="birthdate"]').setValue('1995-06-15')
  ...
  it('생일이 아직 안 지났으면 나이가 1살 적게 계산되어야 한다', async () => {
    ...
    await wrapper.find('input[name="birthdate"]').setValue('1998-12-31')
    expect(wrapper.text()).toContain('만 27세')

    // 1999-02-01 생년월일 입력 → 2026-01-15 기준 만 26세 (아직 생일 안 지남)
    await wrapper.find('input[name="birthdate"]').setValue('1999-02-01')
